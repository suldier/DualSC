...
def HSI_to_superpixels(img, num_superpixel, is_pca=True, is_show_superpixel=False):
    ...
    if is_pca:
        ...
        img = pca.fit_transform(scale(img.reshape(n_row*n_col, -1))).reshape(n_row, n_col, -1)
    #superpixel_label = slic(img[:, :, :3], n_segments=num_superpixel, compactness=10, max_num_iter=10, convert2lab=False,
    #                        enforce_connectivity=True, min_size_factor=0.3, max_size_factor=2, slic_zero=False)
    superpixel_label = slic(img[:,:,:], n_segments=num_superpixel, compactness=0.1, max_num_iter=10, enforce_connectivity=False, min_size_factor=0.3, max_size_factor=2,  slic_zero=False)
    #superpixel_label = slic(img[:,:,:3])#,  slic_zero=False)
    #superpixel_label = slic(img[:, :, :], n_segments=num_superpixel, compactness=0.1, max_num_iter=10, convert2lab=False,
    #                        enforce_connectivity=True, min_size_factor=0.3, max_size_factor=2, slic_zero=False)  # compactness=0.1, 0.01
...
def show_superpixel_gt(label, y_pre, y_indx, gt):
        import copy
        import matplotlib.pyplot as plt
        gt = gt.astype('int16')
        ...
        fig = plt.figure()
        #plt.imshow(gt_pre_2d, cmap='nipy_spectral', interpolation='nearest')  # spectral

        color = (132 / 255, 133 / 255, 135 / 255)
        if gt is not None:
            color = (162/255, 169/255, 175/25)
            #gt_pre_2d = minmax_scale(gt_pre_2d.reshape(label.shape[0] * label.shape[1], -1))
            #gt_pre_2d = gt_pre_2d.reshape(label.shape[0], label.shape[1], -1)
            #mask = mark_boundaries(gt_pre_2d, label, mode='outer')
            mask = mark_boundaries(gt_pre_2d/ gt_pre_2d.max(), label, mode='subpixel')
        ...
        plt.axis('off')
        plt.tight_layout()
        fig.savefig('superpixel_1.pdf', format='pdf', bbox_inches='tight', pad_inches=0, dpi=300)
        plt.show()
...
def create_spixel_graph(source_img, superpixel_labels, n_neighbors):
    ...
    adj = kneighbors_graph(ss_fea, n_neighbors=n_neighbors, mode='connectivity', include_self=False).toarray()   # mode='distance'

    n = ss_fea.shape[0]
    a = np.ones(n) / n

    M = ot.dist(ss_fea, ss_fea)
    return adj, center_indx



def create_spixel_graph1(source_img, superpixel_labels, n_neighbors= 10):
    s = source_img.reshape((-1, source_img.shape[-1]))
    a = create_association_mat(superpixel_labels)
    # t = superpixel_labels.reshape(-1)
    mean_fea = np.matmul(a.T, s)
    ...
    ss_fea = np.concatenate((mean_fea, center_indx), axis=1)
    ss_fea = minmax_scale(ss_fea)
    adj = kneighbors_graph(ss_fea, n_neighbors, mode='connectivity', include_self=False).toarray()
