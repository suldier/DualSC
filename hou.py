...
if __name__ == "__main__":
    ...
    config = yaml_config_hook("config_hou.yaml")
    ...
    dataset.p.show_class_map_color_zone(dataset.gt, save = args.dataset+"_gt_zone.pdf")
    ...
    C, R =  dualsc_with_zero(x, alpha = 0.00001, beta = 0.001, rho=1, max_iter=100)
    y_pred = cal_sp_sc(C, dataset.association_mat, 0.9, dataset.n_classes)
    ...
    print(ca)
    #oa, kappa, nmi, ca, class_map_full = get_acc_map_no_ground(y_pred, dataset.gt)
    #dataset.p.show_class_map_color(class_map_full.reshape(dataset.gt.shape), save = args.dataset+"_dualsc_no_ground.pdf")
