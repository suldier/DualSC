...
CLASS_MAP_COLOR = {
    ...
    '0': [0, 0, 0], '1': [50, 106, 179], '2': [83, 171, 72], '3': [137, 186, 67], '4': [66, 132, 91],
    '5': [60, 131, 200], '6': [144, 82, 54], '7': [105, 188, 200], '8': [254, 0, 250], '9': [199, 176, 201], '10': [218, 51, 44],
...
class Processor:
    ...
    def show_class_map(self, y_pre, y_indx, gt, show=True, save=True):
        """
        :param y_pre:
        ...
        if show:
            plt.show()
    def show_representation(self, C, save=True):
        """
        :param y_pre:
        :param y_indx: predicted pixel index in the ground truth
        :param gt:
        ...
        import matplotlib.pyplot as plt
        fig, ax = plt.subplots()
        ax.imshow(C,interpolation='nearest')  # spectral
        plt.axis('off')
        plt.tight_layout()
        if save is not False:
            plt.savefig(save, format='pdf', bbox_inches='tight',  pad_inches=0.0)

    def show_img(self, img, save=True):
        """
        :param y_pre:
        :param y_indx: predicted pixel index in the ground truth
        :param gt:
        ...
        import matplotlib.pyplot as plt
        fig, ax = plt.subplots()
        ax.imshow(img, interpolation='nearest')  # spectral
        plt.axis('off')
        plt.tight_layout()
        if save is not False:
            plt.savefig(save, format='pdf', bbox_inches='tight',  pad_inches=0.0)
    
    def show_class_map_color(self, class_map, show=False, save=True):
        """
        :param y_pre:
        :param y_indx: predicted pixel index in the ground truth
        :param gt:
        ...
        fig, ax = plt.subplots()
        m, n = class_map.shape
        color_map = np.zeros((m, n, 3), dtype=np.int32)
        for i in range(m):
            for j in range(n):
                color_map[i, j] = CLASS_MAP_COLOR[str(class_map[i, j])]
        ax.imshow(color_map,interpolation='nearest')  # spectral
        plt.axis('off')
        plt.tight_layout()
        if save is not False:
            plt.savefig(save, format='pdf', bbox_inches='tight', pad_inches=0.0, dpi=300)
        if show:
            plt.show()
    def show_class_map_color_zone(self, class_map, show=False, save=True):
        """
        :param y_pre:
        :param y_indx: predicted pixel index in the ground truth
        :param gt:
        ...
        p1=plt.subplot(122)
        p1.imshow(color_map, interpolation='nearest')  # spectral
        #plt.xticks([])
        #plt.yticks([])
        plt.axis('off')
        ...
        p1.plot(sx,sy,"red", linewidth=1)

        #plt.tight_layout()
        p2=plt.subplot(121)
        img2 = color_map[ty0: ty1, tx0:tx1,:]
        p2.imshow(img2,  interpolation='nearest')
        plt.xticks([])
        plt.yticks([])
        p2.spines["left"].set_color("red")
        p2.spines["right"].set_color("red")
        p2.spines["top"].set_color("red")
        p2.spines["bottom"].set_color("red")

        p2.spines["left"].set_linewidth(1)
        p2.spines["right"].set_linewidth(1)
        p2.spines["top"].set_linewidth(1)
        p2.spines["bottom"].set_linewidth(1)

        #p = p2.get_position()
        left, bottom, width, height = p1.get_position().bounds

        p2.set_position([left - 0.26, bottom, width, height])
        left, bottom, width, height = p2.get_position().bounds

        xy=(tx0, ty0)
        xy2 = (100, -0.4)
        #xy2 = (-0.5, -0.4)
        con = ConnectionPatch(xyA=xy2,xyB=xy,coordsA="data",coordsB="data",
                axesA=p2,axesB=p1, color="red", linewidth=1 )
        p2.add_artist(con)
        ...
        con = ConnectionPatch(xyA=xy2,xyB=xy,coordsA="data",coordsB="data",
                axesA=p2,axesB=p1, color="red", linewidth=1)
        p2.add_artist(con)
        plt.savefig(save, format='pdf', bbox_inches='tight', dpi=300, pad_inches=0.01)
    def show_class_map_color_16(self, class_map, show=False, save=True):
        """
        :param y_pre:
        :param y_indx: predicted pixel index in the ground truth
        :param gt:
        ...
        for i in range(m):
            for j in range(n):
                color_map[i, j] = CLASS_MAP_COLOR_16[class_map[i, j]]
        ax.imshow(color_map,interpolation='nearest')  # spectral
        plt.axis('off')
        plt.tight_layout()
        if save is not False:
        ...
        if show:
            plt.show()
    def show_color_label(self,color, save):
        fig = plt.figure()
        ax =  fig.add_subplot(111)

        rec = plt.Rectangle((0.1, 0.1), 0.5, 0.3)
        ax.add_patch(rec)
        plt.savefig(save, format='pdf', bbox_inches='tight', pad_inches=0.01)

    def show_color_label(self, label, idx, save):
        ...
        plt.axis('off')
        x, y = 0, 0.9
        #x, y = 0, (len(label)//2 +1) * 0.1


        for i in range(len(idx)):
            ax.text(x+ 0.11, y+0.03, label[idx[i]])
            rec = plt.Rectangle((x, y), 0.1, 0.1, linewidth=2, edgecolor='red', color=np.array(CLASS_MAP_COLOR[str(idx[i])])/255)
            y -= 0.1
            if i == len(idx)//2:
                x += 0.5
                y = 0.9
